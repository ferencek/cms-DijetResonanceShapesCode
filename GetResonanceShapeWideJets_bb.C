#include "LineShape_pdf_WideJets_bb.h"
...
void GetResonanceShape_bb()
{
    ...
    TFile *output = new TFile("Resonance_Shapes_WideJets_bb.root","RECREATE");
    ...
    for(int j=0; j<36; j++){
       ...
       // make the histograms
       h_pdf[j]=new TH1D(TString(histname)+"_pdf", "b#bar{b} Resonance Shape", 14000, 0, 14000);
       h_cdf[j]=new TH1D(TString(histname)+"_cdf", "b#bar{b} Resonance Shape CDF", 14000, 0, 14000);
