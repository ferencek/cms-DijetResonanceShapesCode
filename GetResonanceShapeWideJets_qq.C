#include "LineShape_pdf_WideJets_qq.h"

void GetResonanceShape_qq()
{
    ...
    TFile *output = new TFile("Resonance_Shapes_WideJets_qq.root","RECREATE");
    ...
    for(int j=0; j<36; j++){
       ...
       sprintf(histname,"h_qq_%d",resmass);

       h_shape[j] = new TH1D(histname,"q#bar{q} Resonance Shape",nMassBins,massBoundaries);
